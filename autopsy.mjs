...
async function runAutopsy() {
  ...
  for (const t of tables) {
    ...
    console.log(
      `Table '${t}':`,
      error ? `ERROR: ${error.message}` : `EXISTS (${data.length} sample rows returned)`,
    );
  }
  ...
  if (pErr) {
    ...
    if (profs.length > 0) {
      ...
      console.log(
        "Sample profile rows:",
        JSON.stringify(
          profs.map((p) => ({ id: p.id, email: p.email, role: p.role, full_name: p.full_name })),
          null,
          2,
        ),
      );
  ...
  console.log(
    "Anon select on profiles:",
    aErr ? `Blocked (${aErr.message})` : `LEAK / OPEN: Returned ${anonProfs.length} rows`,
  );
}
