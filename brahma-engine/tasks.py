...
@celery_app.task(name="tasks.check_tool_health_task", bind=True, queue="default")
def check_tool_health_task(self, tool_id: str, url: str) -> Dict[str, Any]:
    """
    Asynchronous Link Health Telemetry Task with strict SSRF guard.
    Probes external URLs, executes exponential retries, records latency,
    and updates catalog health classification.
    """
    logger.info(f"[TASK:HEALTH] Probing URL health for tool_id={tool_id}, url={url}...")
    result = crawler.probe_tool_url(tool_id, url)
    logger.info(f"[TASK:HEALTH] Completed probe for {tool_id}: status={result['status']}, latency={result['response_time_ms']}ms")
    return result
