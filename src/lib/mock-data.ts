...
export const requirements = {
  functional: [
    { id: "FR-01", text: "Merchants can initiate card, UPI and wallet payments through a single API.", confidence: 96, priority: "Must" },
    { id: "FR-02", text: "System retries failed settlements with idempotency keys for up to 24 hours.", confidence: 91, priority: "Must" },
    { id: "FR-03", text: "Admins can issue full or partial refunds with an audit reason code.", confidence: 88, priority: "Must" },
    { id: "FR-04", text: "Merchants receive webhooks for payment success, failure and chargeback events.", confidence: 84, priority: "Should" },
    { id: "FR-05", text: "Finance team can export daily settlement reconciliation as CSV.", confidence: 79, priority: "Should" },
  ] as RequirementItem[],
  nonFunctional: [
    { id: "NFR-01", text: "p95 authorization latency must stay under 400 ms at 1,200 TPS.", confidence: 92 },
    { id: "NFR-02", text: "All cardholder data must be tokenized; no PAN stored at rest.", confidence: 95 },
    { id: "NFR-03", text: "Availability target of 99.95% per calendar month.", confidence: 86 },
    { id: "NFR-04", text: "Every privileged action must be written to an immutable audit log.", confidence: 82 },
  ] as RequirementItem[],
  actors: [
  ...
  constraints: [
    { id: "CON-01", text: "Must comply with PCI-DSS v4.0 scope reduction guidance.", confidence: 90 },
    { id: "CON-02", text: "Deployment restricted to ap-south-1 region for data residency.", confidence: 85 },
    { id: "CON-03", text: "Existing Oracle ledger must remain the system of record.", confidence: 74 },
  ] as RequirementItem[],
  assumptions: [
    { id: "ASM-01", text: "Acquirer sandbox credentials are available before integration testing.", confidence: 68 },
    { id: "ASM-02", text: "Merchant volume will not exceed 2M transactions per month in year one.", confidence: 61 },
    { id: "ASM-03", text: "KYC verification is handled by an existing internal service.", confidence: 72 },
  ] as RequirementItem[],
  entities: [
    { id: "ENT-01", text: "Merchant, ApiKey, Payment, Refund, SettlementBatch, WebhookEvent, AuditLog", confidence: 88 },
  ] as RequirementItem[],
};
...
export const blueprintNodes = [
  { id: "web", label: "Web Frontend", kind: "Frontend", tech: "React 19 + TypeScript", detail: "Merchant dashboard and admin console. Server-rendered shell with route-level code splitting.", x: 0, y: 0 },
  { id: "gateway", label: "API Gateway", kind: "Backend", tech: "Node.js + Fastify", detail: "Rate limiting, request signing verification and routing to internal services.", x: 260, y: 0 },
  { id: "auth", label: "Authentication", kind: "Auth", tech: "OAuth2 + JWT (RS256)", detail: "Merchant API key exchange, admin SSO and short-lived access tokens.", x: 260, y: -140 },
  { id: "core", label: "Payment Service", kind: "Backend", tech: "Node.js + Prisma", detail: "Authorization, capture, refund state machine with idempotency keys.", x: 520, y: 0 },
  { id: "ai", label: "Risk AI Service", kind: "AI", tech: "Python + FastAPI", detail: "Fraud scoring and delay prediction served over gRPC with a 40 ms budget.", x: 520, y: 150 },
  { id: "db", label: "PostgreSQL", kind: "Database", tech: "Postgres 16 + PgBouncer", detail: "Primary transactional store, logical replication to the reporting replica.", x: 780, y: -80 },
  { id: "worker", label: "Background Worker", kind: "Worker", tech: "BullMQ + Redis", detail: "Settlement batching, webhook retries and reconciliation jobs.", x: 780, y: 80 },
  { id: "storage", label: "File Storage", kind: "Storage", tech: "S3-compatible object store", detail: "Settlement CSVs, dispute evidence and generated PDF reports.", x: 1040, y: 20 },
  { id: "external", label: "Acquirer APIs", kind: "External", tech: "Visa / NPCI / Wallets", detail: "Third-party authorization endpoints with circuit breakers per provider.", x: 780, y: 220 },
];
...
export const apiRoutes = [
  ...
  { method: "GET", path: "/v1/payments/:id", purpose: "Fetch payment state and timeline", auth: true },
  { method: "POST", path: "/v1/payments/:id/capture", purpose: "Capture an authorized payment", auth: true },
  { method: "POST", path: "/v1/refunds", purpose: "Issue a full or partial refund", auth: true },
  { method: "GET", path: "/v1/settlements", purpose: "List settlement batches for a merchant", auth: true },
  { method: "POST", path: "/v1/webhooks/test", purpose: "Send a test webhook to the merchant endpoint", auth: true },
...
export const fileIssues = [
  { file: "src/services/settlement/batchRunner.ts", complexity: 34, issues: 9, severity: "High" as Severity, recommendation: "Extract batching strategy into pure functions and cover with table-driven tests." },
  { file: "src/services/payments/stateMachine.ts", complexity: 27, issues: 6, severity: "High" as Severity, recommendation: "Replace nested conditionals with an explicit transition map." },
  { file: "src/webhooks/dispatcher.ts", complexity: 22, issues: 5, severity: "Medium" as Severity, recommendation: "Move retry backoff into shared utility; add jitter to avoid thundering herd." },
  { file: "src/shared/http/client.ts", complexity: 19, issues: 4, severity: "Medium" as Severity, recommendation: "Add per-host timeouts; current default of 30s exceeds request budget." },
  { file: "src/admin/reconcile/controller.ts", complexity: 16, issues: 3, severity: "Low" as Severity, recommendation: "Split controller from mapping logic to enable unit testing." },
  { file: "src/shared/currency.ts", complexity: 8, issues: 2, severity: "Low" as Severity, recommendation: "Use integer minor units consistently; remove float rounding helper." },
];

// ---------- Security ----------

export const vulnerabilities = [
  { id: "V-101", title: "Hardcoded signing secret in auth module", severity: "Critical" as Severity, cwe: "CWE-798", location: "src/auth/tokens.ts:41", description: "The JWT signing secret is committed as a string literal and reused across environments.", recommendation: "Load the secret from a managed secret store and rotate the exposed key immediately." },
  { id: "V-102", title: "SQL built from unparameterized merchant filter", severity: "Critical" as Severity, cwe: "CWE-89", description: "Merchant search concatenates user input into a raw query string.", location: "src/admin/merchants/search.ts:88", recommendation: "Use parameterized queries or the query builder's bind API." },
  { id: "V-103", title: "Missing authorization check on refund endpoint", severity: "High" as Severity, cwe: "CWE-862", location: "src/routes/refunds.ts:57", description: "Any authenticated merchant can refund a payment belonging to another merchant.", recommendation: "Verify payment.merchant_id equals the caller's merchant before mutating state." },
  { id: "V-104", title: "Verbose error responses leak stack traces", severity: "Medium" as Severity, cwe: "CWE-209", location: "src/shared/errors/handler.ts:23", description: "Unhandled exceptions return the full stack trace to API clients in production.", recommendation: "Return a correlation id and log details server-side only." },
  { id: "V-105", title: "Outdated transitive dependency with known CVE", severity: "Medium" as Severity, cwe: "CWE-1104", location: "package-lock.json", description: "A nested HTTP parser dependency is pinned to a version with a request smuggling advisory.", recommendation: "Upgrade the parent package and add automated dependency scanning to CI." },
  { id: "V-106", title: "Cookie missing SameSite attribute", severity: "Low" as Severity, cwe: "CWE-1275", location: "src/auth/session.ts:12", description: "The admin session cookie sets Secure and HttpOnly but not SameSite.", recommendation: "Set SameSite=Lax for the admin console session cookie." },
];
...
export const riskBusiness = {
  ...
  releaseReadiness: 72,
  kpis: [
    { label: "Cost impact", value: "₹18.4L", note: "Projected rework if debt is unaddressed", tone: "warning" as const },
    { label: "Time impact", value: "+14 days", note: "Estimated schedule slip at current velocity", tone: "warning" as const },
    { label: "Quality impact", value: "-9 pts", note: "Expected drop in release quality index", tone: "critical" as const },
    { label: "Risk exposure", value: "High", note: "Driven by 2 critical security findings", tone: "critical" as const },
    { label: "Maintainability", value: "78 / 100", note: "Above portfolio median of 71", tone: "success" as const },
  ],
};
...
export const reports = [
  { id: "RPT-118", name: "Executive Summary — August", type: "Executive summary", status: "Completed" as const, pages: 14, generated: "2026-08-07 09:40", by: "Priya Nair" },
  { id: "RPT-117", name: "Technical Architecture Review", type: "Technical report", status: "Completed" as const, pages: 42, generated: "2026-08-05 17:11", by: "Arjun Mehta" },
  { id: "RPT-116", name: "Capstone Submission Draft", type: "Academic report", status: "Generating" as const, pages: 0, generated: "2026-08-07 10:02", by: "Priya Nair" },
  { id: "RPT-115", name: "Security Posture Report", type: "Technical report", status: "Failed" as const, pages: 0, generated: "2026-08-04 08:55", by: "System" },
  { id: "RPT-114", name: "Quarterly Business Impact", type: "Executive summary", status: "Completed" as const, pages: 21, generated: "2026-07-30 12:30", by: "Neha Rao" },
];
...
export const adminUsers = [
  { id: "u1", name: "Priya Nair", email: "priya.nair@brahma.dev", role: "Admin", projects: 9, joined: "2026-02-11", status: "Active" },
  { id: "u2", name: "Arjun Mehta", email: "arjun.mehta@aurora.io", role: "Startup", projects: 4, joined: "2026-03-02", status: "Active" },
  { id: "u3", name: "Dr. S. Krishnan", email: "krishnan@univ.edu", role: "Faculty", projects: 12, joined: "2026-01-19", status: "Active" },
  { id: "u4", name: "Rahul Verma", email: "rahul.v@student.univ.edu", role: "Student", projects: 2, joined: "2026-06-27", status: "Invited" },
  { id: "u5", name: "Neha Rao", email: "neha.rao@medisync.health", role: "Startup", projects: 3, joined: "2026-05-14", status: "Suspended" },
];

export const auditLog = [
  { id: "l1", actor: "priya.nair@brahma.dev", action: "Rotated workspace LLM API key", target: "workspace:brahma-core", time: "2026-08-07 09:58" },
  { id: "l2", actor: "system", action: "Security analysis failed after 3 retries", target: "project:vaultledger", time: "2026-08-07 06:12" },
  { id: "l3", actor: "arjun.mehta@aurora.io", action: "Connected GitHub repository", target: "project:brahma-core", time: "2026-08-06 21:44" },
  { id: "l4", actor: "krishnan@univ.edu", action: "Exported academic report", target: "project:campusflow", time: "2026-08-06 15:03" },
  { id: "l5", actor: "priya.nair@brahma.dev", action: "Suspended user account", target: "user:neha.rao", time: "2026-08-05 10:27" },
];
