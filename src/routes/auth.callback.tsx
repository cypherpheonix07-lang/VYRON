    ...
    const handleAuthCallback = async () => {
      try {
        ...
        if (isGitHubIntegration && ghCode) {
          let discoveredAccounts: Array<{ login: string; type: string; avatar_url: string }> = [];
          try {
          ...
          sessionStorage.setItem("brahma_discovered_github_accounts", JSON.stringify(discoveredAccounts));
          if (discoveredAccounts.length === 1) {
            sessionStorage.setItem("brahma_selected_github_account", JSON.stringify(discoveredAccounts[0]));
          }

          const targetProjectId = sessionStorage.getItem("brahma_github_oauth_project_id");
          if (targetProjectId) {
            sessionStorage.removeItem("brahma_github_oauth_project_id");
            navigate({
              to: `/app/studio/${targetProjectId}/integrations`,
              search: { github_flow: "select_account" } as any,
              replace: true,
            });
            return;
          }
