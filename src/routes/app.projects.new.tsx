...
export const Route = createFileRoute("/app/projects/new")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Capture project details, requirements and repository, then generate a validated blueprint.",
      },
...
const stepTitles = ["Project information", "Requirement input", "Repository connection", "Analysis options"];
...
function NewProject() {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [running, progressIndex, navigate]);

  function validate() {
    const next: Record<string, string> = {};
    if (step === 0) {
      if (info.name.trim().length < 3) next["name"] = "Project name must be at least 3 characters";
      if (info.description.trim().length < 20) next["description"] = "Add at least 20 characters of context";
              ...
              {analysisSteps.map((s, i) => {
                ...
                return (
                  <li key={s} className="flex items-center gap-3">
                    ...
                    >
                      {done ? <Check className="size-3.5" aria-hidden /> : active ? <Loader2 className="size-3.5 animate-spin" aria-hidden /> : i + 1}
                    </span>
                    <span className={cn("text-sm", !done && !active && "text-muted-foreground")}>{s}</span>
                  </li>
                );
              })}
            </ol>
      ...
      <ol className="grid gap-2 sm:grid-cols-4">
        {stepTitles.map((t, i) => (
          ...
          >
            ...
            <span className={cn("min-w-0 truncate text-sm", i === step ? "font-medium" : "text-muted-foreground")}>
              {t}
            </span>
          </li>
        ))}
        ...
        <CardContent className="space-y-5 pt-2">
          {step === 0 ? (
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <Input id="pname" className="mt-1.5" value={info.name} placeholder="Aurora Payments Gateway"
                  onChange={(e) => setInfo((v) => ({ ...v, name: e.target.value }))} />
                {errors["name"] ? <p className="mt-1.5 text-xs text-[var(--critical)]">{errors["name"]}</p> : null}
              </div>
              <div className="sm:col-span-2">
                <Label htmlFor="pdesc">Description</Label>
                <Textarea id="pdesc" className="mt-1.5" rows={3} value={info.description}
                  placeholder="Payment orchestration service for mid-market merchants…"
                  onChange={(e) => setInfo((v) => ({ ...v, description: e.target.value }))} />
                {errors["description"] ? <p className="mt-1.5 text-xs text-[var(--critical)]">{errors["description"]}</p> : null}
              </div>
              <div>
                <Label htmlFor="pdomain">Domain</Label>
                <Select value={info.domain} onValueChange={(v) => setInfo((p) => ({ ...p, domain: v }))}>
                  <SelectTrigger id="pdomain" className="mt-1.5 w-full"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {["Fintech", "Healthcare", "Education", "Logistics", "Enterprise Ops", "E-commerce"].map((d) => (
              ...
              <div>
                ...
                <Input id="pusers" className="mt-1.5" value={info.users} placeholder="Merchant developers, finance operators"
                  onChange={(e) => setInfo((v) => ({ ...v, users: e.target.value }))} />
                {errors["users"] ? <p className="mt-1.5 text-xs text-[var(--critical)]">{errors["users"]}</p> : null}
              </div>
              <div>
                <Label htmlFor="pteam">Team size</Label>
                <Input id="pteam" type="number" min={1} className="mt-1.5" value={info.team}
                  onChange={(e) => setInfo((v) => ({ ...v, team: e.target.value }))} />
              </div>
              <div>
                <Label htmlFor="pdl">Expected deadline</Label>
                <Input id="pdl" type="date" className="mt-1.5" value={info.deadline}
            ...
            <div className="space-y-4">
              <div>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => setIdea(sampleIdea)}>
                    <Sparkles className="size-4" aria-hidden /> Load sample project idea
                  </Button>
                </div>
                <Textarea id="idea" rows={9} className="mt-1.5 font-mono text-xs" value={idea}
                  placeholder="Describe what the system must do, who uses it, and any constraints…"
                  onChange={(e) => setIdea(e.target.value)} />
                {errors["idea"] ? <p className="mt-1.5 text-xs text-[var(--critical)]">{errors["idea"]}</p> : null}
              </div>
          ...
          {step === 2 ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Button type="button" variant="outline" className="h-auto justify-start gap-3 px-4 py-3"
                  onClick={() => setRepo({ url: "github.com/aurora-labs/payments-gateway", connected: true })}>
                ...
                <Button type="button" variant="outline" className="h-auto justify-start gap-3 px-4 py-3"
                  onClick={() => setRepo({ url: "github.com/project-brahma/sample-service", connected: true })}>
                  <Wand2 className="size-4 shrink-0" aria-hidden />
                  <span className="text-left">
                    <span className="block text-sm font-medium">Use sample repository</span>
                    <span className="block text-xs text-muted-foreground">Preloaded analysis data</span>
                  </span>
                </Button>
              </div>
              <div>
                ...
                <div className="mt-1.5 flex gap-2">
                  <Input id="repourl" value={repo.url} placeholder="https://github.com/org/repo"
                    onChange={(e) => setRepo((r) => ({ ...r, url: e.target.value }))} />
                  <Button type="button" variant="secondary" onClick={() => setRepo((r) => ({ ...r, connected: !!r.url.trim() }))}>
                    Connect
                  </Button>
                </div>
          ...
          {step === 3 ? (
            <div className="space-y-1">
              {[
                ["architecture", "Architecture generation", "Service topology, schema and API contract"],
                ["codeHealth", "Code health analysis", "Maintainability, complexity, duplication, coverage"],
                ...
                <div key={key} className="flex items-center justify-between gap-4 border-b border-border/60 py-3 last:border-0">
              ...
              {errors["options"] ? <p className="text-xs text-[var(--critical)]">{errors["options"]}</p> : null}
            </div>
          ) : null}
