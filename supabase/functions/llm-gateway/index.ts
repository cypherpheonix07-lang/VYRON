...
function generateTemplateArtifact(task: string, payload: any): any {
  const prompt = (payload?.prompt || payload?.code || payload?.content || JSON.stringify(payload)).toLowerCase();
  ...
  return {
    response: "PROJECT BRAHMA verified intelligence engine online. All services operational and telemetry actively monitored.",
...
export default {
  fetch: withSupabase({ auth: "none" }, async (req, ctx) => {
    ...
    if (req.method !== "POST") {
      return new Response(
        JSON.stringify({ ok: false, error: { code: "BRA-405", message: "Method not allowed. Use POST." } }),
    ...
    try {
      ...
      if (!task) {
        return new Response(
          JSON.stringify({ ok: false, error: { code: "BRA-400", message: "Missing required 'task' identifier." } }),
          { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } },
        );
      }

      // 3. Check VITE_FORCE_TEMPLATE dev flag (for offline drill)
      const forceTemplate = options?.force_template === true || Deno.env.get("FORCE_TEMPLATE") === "true";
      ...
      try {
        ...
        if (usageRows && usageRows.length > 0) {
          userDailySpend = usageRows.reduce((acc: number, r: { cost_usd: number }) => acc + (Number(r.cost_usd) || 0), 0);
        }
      } catch {
      ...
      else if (preferredTier === "free") chosenOpenRouterModel = "meta-llama/llama-3.1-8b-instruct:free";
...
Analyze the user request and return STRICT valid JSON matching the system schema with no markdown wrappers or explanations.`;
      ...
      const latencyMs = Date.now() - startTime;
      const pricing = MODEL_PRICING[activeModel] || { promptUsdPer1k: 0.0, completionUsdPer1k: 0.0 };
      const calculatedCostUsd =
        (promptTokens / 1000) * pricing.promptUsdPer1k + (completionTokens / 1000) * pricing.completionUsdPer1k;
