...
export default {
  fetch: withSupabase({ auth: "none" }, async (req, ctx) => {
    ...
    if (req.method !== "POST") {
      return new Response(
        JSON.stringify({ error: { code: "BRA-405", message: "Method not allowed" } }),
        {
          status: 405,
    ...
    try {
      ...
      if (!texts || !Array.isArray(texts) || texts.length === 0) {
        return new Response(
          JSON.stringify({
            error: { code: "BRA-400", message: "Missing or invalid 'texts' array parameter." },
          }),
          {
            status: 400,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
      ...
      try {
        ...
        console.warn(
          "HF Inference network/DNS exception, falling back to deterministic embeddings:",
          netErr,
        );
        const fallbackEmbeddings = texts.map((t) => generateDeterministicEmbedding(t));
        return new Response(
          JSON.stringify({
            ...
            dim: 384,
            provider: "deterministic-fallback",
            warning: "Hugging Face network error. Fallen back to deterministic feature vectors.",
          }),
          {
            status: 200,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
      ...
      if (!hfRes.ok) {
        ...
        // Graceful fallback for rate limits or model load
        const fallbackEmbeddings = texts.map((t) => generateDeterministicEmbedding(t));
        return new Response(
          JSON.stringify({
            ok: true,
            ...
            dim: 384,
            provider: "deterministic-fallback",
            warning: `Hugging Face returned ${hfRes.status}. Fallen back to deterministic feature vectors.`,
          }),
          {
            status: 200,
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
