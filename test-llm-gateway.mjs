...
async function testG3() {
  try {
    ...
    const hasCacheTable =
      migration.includes("public.llm_cache") && migration.includes("vector(384)");
    const hasCacheLookup =
      edgeSource.includes("llm_cache") && edgeSource.includes("cache_hit: true");
...
async function testG5() {
  try {
    ...
    const hasDedupLogic =
      reqTabSource.includes("llmGateway.embed") && reqTabSource.includes("cosineSimilarity");

    if (hasDedupLogic && sim12 >= 0.9 && sim13 < 0.85) {
      recordResult(
        "G5",
      ...
      recordResult(
        ...
        false,
        "Dedup logic missing in requirements tab",
      );
    }
  } catch (err) {
...
async function testG8() {
  try {
    ...
    if (popoverExists && reqTabUpdated && blueprintUpdated) {
      ...
      recordResult(
        "G8",
        "Provenance UI Integration",
        false,
        "Missing popover or route integration",
      );
    }
  } catch (err) {
